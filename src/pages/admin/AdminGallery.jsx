...
const AdminGallery = () => {
  ...
  const [form, setForm] = useState({ title: '', category: 'Training', media_type: 'image', display_order: 0 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalOpen(false);
      setForm({ title: '', category: 'Training', media_type: 'image', display_order: 0 });
      setFile(null);
      load();
    } catch (err) {
  ...
  return (
    <AdminDashboardLayout>
      <AdminPageHeader title="Gallery" subtitle="Images and videos, uploaded via Cloudinary." actionLabel="Upload Media" onAction={() => setModalOpen(true)} />
          ...
          {items.map((item) => (
            <div key={item.id} className="card overflow-hidden relative group">
              ...
              <div className="p-3">
                ...
                <button onClick={() => setConfirmId(item.id)} className="text-maroon-400 text-xs hover:underline mt-2">Delete</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Upload Media">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={saving} className="btn-primary w-full disabled:opacity-60">{saving ? 'Uploading…' : 'Upload'}</button>
        </form>
      </Modal>
